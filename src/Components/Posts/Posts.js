import React from "react";
import "./Post.css";

const Posts = () => {
  return (
    <div className="block md:grid grid-cols-2 p-6">
      {/* Repeat Start */}
      <div className="post p-5 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(75).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 right-10">
          <p className="post-text text-right font-bold text-5xl">
            The Snow Path
          </p>
          <p className="post-text text-right font-bold text-3xl">Siberia</p>
        </div>
      </div>
      {/* Repeat End */}

      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(76).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(77).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(78).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(79).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(74).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(73).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(72).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(71).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(70).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(70).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(70).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(70).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(70).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(70).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(70).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(70).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(70).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(70).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
      <div className="post p-2 relative cursor-pointer">
        <img
          className="block object-cover object-center w-full h-full rounded-lg transition duration-500"
          src="https://mdbcdn.b-cdn.net/img/Photos/Horizontal/Nature/4-col/img%20(70).webp"
          alt=""
        />
        <div className="post-text-box absolute invisible bottom-8 left-8">
          <p className="post-text font-bold text-2xl">The Snow Path</p>
          <p className="post-text font-bold">Siberia</p>
        </div>
      </div>
    </div>
  );
};

export default Posts;
